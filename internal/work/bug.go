...
func (s *Service) CreateBug(ctx context.Context, opts CreateBugOptions) (*CreateBugResult, error) {
	...
	managed := bugNotes(CreateBugOptions{Priority: opts.Priority, Environment: opts.Environment})
	...
	if epic == nil {
		return nil, output.NewError("EPIC_NOT_FOUND", "Epic not found.")
	}
	...
	task, err := s.asana().CreateTask(ctx, resolved.Token, asana.CreateTaskInput{
		...
		Notes:        notes,
		HTMLNotes:    htmlNotes,
		CustomFields: creationCustomFields(cfg, cfg.TaskTypes.Bug, true),
	})
	if err != nil {
