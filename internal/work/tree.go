...
func (s *Service) WorkTree(ctx context.Context, opts WorkTreeOptions) (*WorkTreeResult, error) {
	...
	epicGIDs := sortedTreeGIDs(epics)
	for _, epicGID := range epicGIDs {
		epic := epics[epicGID]
...
func sortedTreeNodes(nodes map[string]*TreeNode) []TreeNode {
	...
	for _, gid := range sortedTreeGIDs(nodes) {
		node := nodes[gid]
		out = append(out, *node)
	}
	sortTreeNodes(out)
...
func sortedTreeGIDs(nodes map[string]*TreeNode) []string {
	gids := make([]string, 0, len(nodes))
	for gid := range nodes {
...
func treeGIDSet(epics map[string]*TreeNode) map[string]bool {
	out := map[string]bool{}
	for _, epic := range epics {
		addTreeGIDs(out, epic)
	}
	return out
}

func addTreeGIDs(out map[string]bool, node *TreeNode) {
	if node == nil || node.Item.GID == "" {
		return
	}
	out[node.Item.GID] = true
	for i := range node.Children {
		addTreeGIDs(out, &node.Children[i])
	}
}
