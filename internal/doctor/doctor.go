...
func finish(checks []Check, repairPlan bool, repairDryRun bool, authSource string, contextName string) *Result {
	result := &Result{OK: true, Checks: checks, EffectiveAuthSource: authSource, EffectiveContext: contextName, CapabilitySchema: "mvp-plus-6", CheckedAt: time.Now().UTC().Format(time.RFC3339)}
